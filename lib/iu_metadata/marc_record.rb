...
#FIXME: use date, or new/different method?
    ...
    def formatted_subfields_as_array(fields, opts = {})
      vals = []
      data.fields(fields).each do |field|
        val = format_datafield_subfields(field, opts)
        vals += val if val.present?
        next unless linked_field?(field)
        linked_field = get_linked_field(field)
        val = format_datafield_subfields(linked_field, opts)
        vals += val if val.present?
      end
      vals
    end
    ...
    def format_datafield(datafield, hsh = {})
      ...
      format_datafield_subfields(datafield, hsh).join(separator)
    end

    def format_datafield_subfields(datafield, hsh = {})
