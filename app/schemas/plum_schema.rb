...
class PlumSchema < ActiveTriples::Schema
  ...
  def self.edit_fields
    # remove fields handled elsewhere
    PlumSchema.display_fields - [:has_model, :depositor, :date_uploaded, :date_modified, :relative_path, :import_url, :replaces, :source_metadata] - [:portion_note, :rights_note, :start_canvas]
    # hard-coded list
    [:identifier, :related_url, :sort_title, :responsibility_note, :series, :creator, :published, :publication_place, :publisher, :issued, :physical_description, :lccn_call_number, :local_call_number, :copyright_holder, :date_created, :subject]
  end
end
